...
// Engine-internal escape hatch: ecs_api.h's FeatherWorld/FeatherEntity carry
...
// exactly what an EcsModule::on_import(FeatherWorld, FeatherEntity)
...
// owns.
inline World unwrap(ecs::FeatherWorld w) {
	return World(static_cast<ecs_world_t*>(w._handle));
}
inline Entity unwrap(ecs::FeatherEntity e) {
