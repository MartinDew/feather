...
-- whichever project is top-level for the current invocation, which is a
-- CONSUMER's repo if this file is ever includes()'d cross-repo (it is loaded
-- by the root xmake.lua today, but feather_module_target() below is also
-- what a consumer's static build would need to fold modules in -- see the
-- plugin-abi-rework plan's static-shipping stage). Captured once as a
-- description-scope local and closed over by the after_build callback below
-- -- verified on this box's xmake (3.1.0) that the closure keeps this
-- upvalue intact even inside a sandboxed callback and across a cross-repo
-- includes(); it is only a BARE os.projectdir()/os.scriptdir() call
-- re-evaluated fresh *inside* the callback that resolves against the wrong
-- project.
