...
package("vulkan-loader")
    ...
    on_install("mingw", function (package)
        ...
        import("net.http")
        http.download(def_url, def_file)
        assert(os.isfile(def_file), "failed to download vulkan-1.def")

        local libdir = package:installdir("lib")
        os.mkdir(libdir)
        local dlltool = package:build_getenv("dlltool") or package:tool("dlltool")
        if not dlltool then
            local mingw_prefix = "x86_64-w64-mingw32-"
            if package:arch() and package:arch():find("i386", 1, true) then
        ...
        os.vrunv(dlltool, {
            "-d", def_file,
            "-D", "vulkan-1.dll",
            "-l", path.join(libdir, "libvulkan-1.dll.a"),
        })
    end)

    on_fetch("mingw", function (package)
        local libfile = path.join(package:installdir("lib"), "libvulkan-1.dll.a")
    ...
    on_test(function (package)
        if package:is_plat("mingw") then
            assert(os.isfile(path.join(package:installdir("lib"), "libvulkan-1.dll.a")))
        else
