// C++ test plugin for feather::ext::ecs (the typed sugar generated on top of
...
void system_a(Counter& c, float) {
	...
	++a_ticks;
	if (a_ticks == 1)
		feather::ext::_bindings::log("ecs_sugar_test plugin: system_a ticked");
}
...
void system_b(Counter& c, float) {
	...
	++b_ticks;
	if (b_ticks == 1)
		feather::ext::_bindings::log("ecs_sugar_test plugin: system_b ticked");
}
...
void initialize(void*, FeatherInitializationLevel level) {
	...
	feather::ext::ecs::register_component<Counter>("Counter");
	feather::ext::ecs::each<Counter>("SystemA", FEATHER_PHASE_ON_UPDATE, &system_a);
	feather::ext::ecs::each<Counter>("SystemB", FEATHER_PHASE_ON_UPDATE, &system_b);
	feather::ext::ecs::spawn<Counter>("counter_entity", Counter { 0 });

	feather::ext::_bindings::log("ecs_sugar_test plugin: registered two systems over the same component type");
}
...
extern "C" FEATHER_EXTENSION_EXPORT bool feather_extension_init(
	...
	feather::ext::_bindings::log("ecs_sugar_test plugin: feather_extension_init");
